...
**Importante:** este teste usa OCR clássico. Modelos de IA multimodais (que "leem" a página como imagem, não via extração de texto) tendem a ser mais robustos a esse tipo de distorção, porque foram treinados com grandes volumes de imagens degradadas, incluindo CAPTCHAs. O nível `agressivo` reduz bastante a legibilidade pra ambos, mas não existe garantia de que ele derrote uma IA de visão moderna do mesmo jeito que derrota o Tesseract.
...
- **Modelos de IA multimodais podem ser mais resistentes que o OCR clássico testado aqui** (ver "Resultados medidos" acima) - os números desta seção não devem ser lidos como "X% de proteção contra IA" de forma genérica.
